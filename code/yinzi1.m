...
Lf=10.*log10((abs(Rv)).^2*18/23+(abs(Rh)).^2.*5/23)
%µçÀë²ãËðºÄ
R=6370;
R12=100;fh=1.2;
X=0.5;
...
Ej=5:30;
SNR=20*log10(E./Ej);
plot(Ej,SNR,'linewidth',2)
xlabel('Atmospheric Noise Intensity (dB)')
ylabel('SNR (dB)')
hold on
d1=d/2;r=2*R.*(sin(d1./(2*R))./cos(sig1+(d1./(2*R))));
Lbf2=32.44+20.*log10(f)+20*log10(r)
%·´Éä²ãËðºÄ
a2=80;ro=4;bc=3*10^2/17.68;
a1=a2-60*bc*ro*i;
Rv=(a1.*sin(b)-sqrt(a1-(cos(b).^2)))./(a1.*sin(b)+sqrt(a1-(cos(b).^2)));
...
E2=136.6+Pt+Gt+20.*log10(f)-2*Lbf2-Lf-Li-Lf-Li2;
SNR2=20*log10(E2./Ej);
plot(Ej,SNR2,'linewidth',2)
